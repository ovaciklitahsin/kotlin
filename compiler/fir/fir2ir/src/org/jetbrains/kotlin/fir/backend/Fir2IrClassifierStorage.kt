...
class Fir2IrClassifierStorage(
    ...
    private fun createLocalIrClassOnTheFly(klass: FirClass): IrClass {
        ...
        val result = if (klass is FirAnonymousObject) {
            createIrAnonymousObject(klass, irParent = temporaryParent).apply {
            ...
            val irClass = registerIrClass(regularClass, temporaryParent, origin)
            processClassHeader(regularClass, irClass)
            converter.processClassMembers(regularClass, irClass)
            irClass
        }
