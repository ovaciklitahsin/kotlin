...
internal typealias Fe10RendererContext = StringBuilder

internal class Fe10DeclarationRenderer(
    ...
    private val typeRenderer = Fe10TypeRenderer(options.typeRendererOptions, isDebugText)
    ...
    private fun Fe10RendererContext.curlyBlock(body: Fe10RendererContext.() -> Unit) {
        append(" {")
        indentedBlock(body)
        appendLine()
        renderIndentation()
    ...
    private fun Fe10RendererContext.indentedBlock(body: Fe10RendererContext.() -> Unit) {
        indentation += 1
        body()
        indentation -= 1
    }

    fun render(descriptor: DeclarationDescriptor, consumer: Fe10RendererContext) {
        consumer.renderDeclaration(descriptor)
    }

    private fun Fe10RendererContext.renderType(type: KotlinType, shouldApproximate: Boolean = false) {
    ...
    private fun Fe10RendererContext.renderDeclaration(descriptor: DeclarationDescriptor) {
        when (descriptor) {
    ...
    private fun Fe10RendererContext.renderClassifier(descriptor: ClassifierDescriptor) {
        when (descriptor) {
    ...
    private fun Fe10RendererContext.renderTypeAlias(descriptor: TypeAliasDescriptor) {
    ...
    private fun Fe10RendererContext.renderTypeParameters(typeParameters: List<TypeParameterDescriptor>) {
        renderList(typeParameters, separator = ", ", prefix = "<", postfix = ">", renderWhenEmpty = false) { renderTypeParameter(it) }
    }

    private fun Fe10RendererContext.renderTypeParameter(descriptor: TypeParameterDescriptor) {
    ...
    private fun Fe10RendererContext.renderClass(descriptor: ClassDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Fe10RendererContext.renderSupertypes(descriptor: ClassDescriptor) {
    ...
    private fun Fe10RendererContext.renderCallable(descriptor: CallableDescriptor) {
        when (descriptor) {
    ...
    private fun Fe10RendererContext.renderPropertyAccessor(descriptor: PropertyAccessorDescriptor) {
        appendLine()
        indentedBlock {
            renderIndentation()
            if (options.renderDeclarationHeader) {
    ...
    private fun Fe10RendererContext.renderEnumEntry(descriptor: ClassDescriptor) {
    ...
    private fun Fe10RendererContext.renderLocalVariable(descriptor: LocalVariableDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Fe10RendererContext.renderProperty(descriptor: PropertyDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Fe10RendererContext.renderReceiver(descriptor: CallableDescriptor) {
    ...
    private fun Fe10RendererContext.renderConstructor(descriptor: ConstructorDescriptor) {
        appendLine()
        renderIndentation()
        if (options.renderDeclarationHeader) {
            renderAnnotations(descriptor.annotations)
    ...
    private fun Fe10RendererContext.renderFunction(descriptor: FunctionDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Fe10RendererContext.renderValueParameters(valueParameters: List<ValueParameterDescriptor>) {
        renderList(valueParameters, separator = ", ", prefix = "(", postfix = ")", renderWhenEmpty = true) { renderValueParameter(it) }
    }

    private fun Fe10RendererContext.renderValueParameter(descriptor: ValueParameterDescriptor, name: Name = descriptor.name) {
        if (options.renderDeclarationHeader) {
            renderAnnotations(descriptor.annotations)
        }
    ...
    private fun Fe10RendererContext.renderLocalDeclarations(descriptor: CallableMemberDescriptor) {
        if (!options.renderContainingDeclarations) {
            return
        }
    ...
    private fun Fe10RendererContext.renderName(descriptor: DeclarationDescriptor) {
        append(descriptor.name.render())
    }

    private fun Fe10RendererContext.renderAnnotations(annotations: Annotations, predicate: (ClassId) -> Boolean = { true }) {
    ...
    private fun Fe10RendererContext.renderModifiers(descriptor: DeclarationDescriptor) {
    ...
    private fun Fe10RendererContext.renderVisibility(descriptor: DeclarationDescriptorWithVisibility) {
    ...
    private fun Fe10RendererContext.renderModality(descriptor: MemberDescriptor) {
    ...
    private fun Fe10RendererContext.renderModifier(text: String, state: Boolean, modifier: RendererModifier? = null) {
        if (state && (modifier == null || modifier in options.modifiers)) {
            append(text).append(' ')
        }
    }

    private fun Fe10RendererContext.renderIndentation() {
        append(INDENTATION.repeat(indentation))
    }
}

internal fun <T> Fe10RendererContext.renderList(
    list: Collection<T>,
    separator: String,
    prefix: String = "",
    postfix: String = "",
    renderWhenEmpty: Boolean = true,
    renderItem: Fe10RendererContext.(T) -> Unit
) {
