...
internal class Fe10TypeRenderer(private val options: KtTypeRendererOptions, private val isDebugText: Boolean = false) {
    private companion object {
        const val ERROR_TYPE_TEXT = "ERROR_TYPE"
    }

    fun render(type: KotlinType, consumer: Fe10RendererContext) {
        consumer.renderType(type)
    }

    private fun Fe10RendererContext.renderType(type: KotlinType) {
        if (isDebugText) {
    ...
    private fun Fe10RendererContext.renderTypeAnnotationsDebug(type: KotlinType) {
    ...
    private fun Fe10RendererContext.renderTypeAnnotationDebug(annotation: AnnotationDescriptor) {
    ...
    private fun Fe10RendererContext.renderAnnotationDebug(classId: ClassId?, namedValues: List<KtNamedConstantValue>) {
        append("@")
    ...
    private fun Fe10RendererContext.renderConstantValueDebug(value: KtConstantValue) {
        when (value) {
    ...
    private fun Fe10RendererContext.renderFlexibleType(type: FlexibleType) {
        if (isDebugText) {
            append("ft<")
    ...
    private fun Fe10RendererContext.renderDefinitelyNotNullType(type: DefinitelyNotNullType) {
        renderType(type.original)
        append(" & Any")
    }

    private fun Fe10RendererContext.renderErrorType() {
        append(ERROR_TYPE_TEXT)
    }

    private fun Fe10RendererContext.renderCapturedType(type: CapturedType) {
        append("CapturedType(")
        renderTypeProjection(type.typeProjection)
        append(")")
    }

    private fun Fe10RendererContext.renderCapturedType(type: NewCapturedType) {
        append("CapturedType(")
        renderTypeProjection(type.constructor.projection)
        append(")")
    }

    private fun Fe10RendererContext.renderTypeVariableType(typeConstructor: NewTypeVariableConstructor) {
    ...
    private fun Fe10RendererContext.renderIntersectionType(typeConstructor: IntersectionTypeConstructor) {
        if (isDebugText) {
            append("it")
        }
    ...
    private fun Fe10RendererContext.renderFunctionType(type: SimpleType) {
    ...
    private fun Fe10RendererContext.renderTypeParameterType(descriptor: TypeParameterDescriptor) {
        append(descriptor.name.render())
    }

    private fun Fe10RendererContext.renderOrdinaryType(type: SimpleType) {
        val nestedType = Fe10JvmTypeMapperContext.getNestedType(type)
    ...
    private fun Fe10RendererContext.renderTypeSegment(typeSegment: PossiblyInnerType, isRoot: Boolean = false) {
    ...
    private fun Fe10RendererContext.renderFqName(fqName: FqName) {
        renderList(fqName.pathSegments(), separator = ".") { append(it.render()) }
    }

    private fun Fe10RendererContext.renderTypeProjection(projection: TypeProjection) {
